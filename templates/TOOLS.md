...
- Use `list_files` to inspect directories inside the workspace.
- Prefer this over `run_shell` when you only need file paths.

## search - Content Search

- Use `search` to search file contents inside the workspace.
- Prefer this over `run_shell` for code and history searches.
...
## write_file and patch_file - File Editing

- Use `patch_file` for targeted replacements in existing files.
...
- Use `run_shell` for tests, scripts, and commands that cannot be handled by file/search tools.
